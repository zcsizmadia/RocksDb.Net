...
| `WriteBatchWithIndex.DeleteRange` throws `NotSupportedException` and is marked obsolete | RocksDb does not support it on an indexed batch: the native call returns `NotSupported` for every argument and the C API discards that status, so the method queued nothing and reported success. Use `WriteBatch.DeleteRange` |
